...
// use crate::common::constants::LOAD_INDEX_MUTEX;
...
pub fn load_index_reader(index_path: &str) -> Result<bool, TantivySearchError> {
    ...
    // Require `index_path` lock.
    // let path_locks_pin = FFI_INDEX_SEARCHER_CACHE.path_locks.pin();
    // let lock = match path_locks_pin.get(&index_path.to_string()) {
    //     Some(lock) => lock,
    //     None => {
    //         path_locks_pin.insert(index_path.to_string(), Mutex::new(()));
    //         path_locks_pin.get(index_path).unwrap()
    //     }
    // };
    // let _guard = lock.lock().unwrap();

    let already_exist = FFI_INDEX_SEARCHER_CACHE.get_index_reader_bridge(index_path.to_string());

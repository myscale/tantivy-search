...
impl IndexReaderBridgeCache {
    ...
    pub fn remove_index_reader_bridge(&self, key: String) -> Result<(), String> {
        let pinned = self.cache.pin();
        // let pinned_path_lock = self.path_locks.pin();
        let trimmed_key: String = key.trim_end_matches('/').to_string();
        if pinned.contains_key(&trimmed_key) {
        ...
        // if pinned_path_lock.contains_key(&trimmed_key) {
        //     pinned_path_lock.remove(&trimmed_key);
        // } else {
        //     let message: String = format!(
        //         "Lock doesn't exist, can't remove it with given key [{}]",
        //         trimmed_key
        //     );
        //     DEBUG!("{}", message);
        //     return Err(message);
